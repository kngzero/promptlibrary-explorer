...
const NavItem: React.FC<{
    ...
    dragSourcePath: string | null;
    isDragActive: boolean;
}> = ({ icon, label, onClick, isSelected, path, onMoveItem, dragSourcePath, isDragActive }) => {
    const [isDraggingOver, setIsDraggingOver] = useState(false);
    const dragDepthRef = useRef(0);

    const isDropTarget = !!(path && onMoveItem);
    ...
    const dropClasses = isDraggingOver
        ? 'border-2 border-fuchsia-400 bg-fuchsia-500/10 text-white'
        : isDragActive && isDropTarget
            ? 'border-2 border-dashed border-zinc-600/80 bg-zinc-800/60'
    ...
    return (
        <li>
            <button
                ...
                className={`w-full flex items-center gap-3 p-2 text-left rounded-md text-sm transition-all duration-200 ${isSelected ? 'bg-fuchsia-600/20 text-fuchsia-300' : 'text-zinc-300 hover:bg-zinc-700/50'
                    } ${dropClasses}`}
            >
...
const FileTree: React.FC<FileTreeProps> = ({ rootPath, folderTree, selectedPath, onSelect, onSelectFavorite, onMoveItem, dragSourcePath, isDragActive }) => {
            ...
            <nav className="flex flex-col space-y-4 h-full">
                <div>
                    ...
                    <ul className="space-y-1">
                        <NavItem icon={<DesktopIcon />} label="Desktop" onClick={() => onSelectFavorite('desktop')} isSelected={false} dragSourcePath={null} isDragActive={false} />
                        <NavItem icon={<DocumentsIcon />} label="Documents" onClick={() => onSelectFavorite('documents')} isSelected={false} dragSourcePath={null} isDragActive={false} />
                        <NavItem icon={<PicturesIcon />} label="Pictures" onClick={() => onSelectFavorite('pictures')} isSelected={false} dragSourcePath={null} isDragActive={false} />
                    </ul>
                </div>
