...
#[tauri::command]
fn generate_thumbnail(source_path: String, dest_path: String, max_size: u32) -> Result<bool, String> {
  ...
  // FilterType::Lanczos3 is high quality but slower, FilterType::Triangle is faster.
  // Using Thumbnail which is optimized for this.
  let thumbnail = img.thumbnail(max_size, max_size);
  
  // Save the result
  thumbnail.save(dest).map_err(|err| err.to_string())?;
