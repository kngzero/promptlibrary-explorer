...
const App: React.FC = () => {
  ...
  const [lastNavScope, setLastNavScope] = useState<'folders' | 'items' | null>(null);
  ...
  const sidebarFolderPaths = useMemo(() => {
    if (!explorerRootPath) return [];
    return [explorerRootPath, ...folderTree.map((dir) => dir.path)];
  }, [explorerRootPath, folderTree]);

  const handleSelectFolderFromSidebar = useCallback(
    (path: string) => {
      setLastNavScope('folders');
      handleSelectFolder(path);
    },
    [handleSelectFolder]
  );

  const handleSelectFavoriteFromSidebar = useCallback(
    async (favorite: 'desktop' | 'documents' | 'pictures') => {
      setLastNavScope('folders');
      return handleSelectFavorite(favorite);
    },
    [handleSelectFavorite]
  );

  const handleItemClickWithScope = useCallback(
    (item: FsFileEntry, index: number, event?: React.MouseEvent) => {
      setLastNavScope('items');
  ...
  useEffect(() => {
    const handleDragStart = (event: DragEvent) => {
      ...
      const hasAllowedSource = eventPath.some(
        (node) =>
          node instanceof Element &&
          (node.matches('input, textarea, [contenteditable="true"], [role="textbox"]') ||
            node.matches('[data-draggable-item="true"]'))
      );

      if (hasAllowedSource) return;

      // Only block default drags from native-draggable elements.
      const target = event.target;
      const isNativeDraggableElement =
        target instanceof HTMLImageElement || target instanceof HTMLAnchorElement;

      if (isNativeDraggableElement) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isTextInput || lightboxOpen) return;
      ...
      if (target?.closest('[data-content-grid="true"]')) return;

      if (lastNavScope === 'folders') {
        if (!sidebarFolderPaths.length) return;
        if (e.key !== 'ArrowUp' && e.key !== 'ArrowDown') return;
        const currentIndex = selectedFolderPath
          ? sidebarFolderPaths.indexOf(selectedFolderPath)
          : -1;
        const safeIndex = currentIndex >= 0 ? currentIndex : 0;
        const delta = e.key === 'ArrowUp' ? -1 : 1;
        const nextIndex = Math.min(sidebarFolderPaths.length - 1, Math.max(0, safeIndex + delta));
        if (nextIndex !== safeIndex) {
          e.preventDefault();
          handleSelectFolderFromSidebar(sidebarFolderPaths[nextIndex]);
        }
        return;
      }

      if (lastNavScope === 'items') {
        if (!processedFolderContents.length) return;
        const grid = document.querySelector<HTMLElement>('[data-content-grid="true"]');
        const columnCount = grid
          ? Math.max(1, window.getComputedStyle(grid).getPropertyValue('grid-template-columns').split(' ').length)
          : 1;
        const currentIndex = selectedItemIndex >= 0 ? selectedItemIndex : 0;
        let nextIndex = currentIndex;
        switch (e.key) {
          case 'ArrowRight':
            nextIndex = Math.min(currentIndex + 1, processedFolderContents.length - 1);
            break;
          case 'ArrowLeft':
            nextIndex = Math.max(currentIndex - 1, 0);
            break;
          case 'ArrowDown':
            nextIndex = Math.min(currentIndex + columnCount, processedFolderContents.length - 1);
            break;
          case 'ArrowUp':
        ...
        if (nextIndex !== currentIndex) {
          ...
          if (e.shiftKey) {
            ...
            handleItemClick(processedFolderContents[nextIndex], nextIndex, syntheticEvent);
          } else {
            handleSelectItemByIndex(nextIndex);
          }
        }
      }
    };
  ...
  const handleCustomReorder = useCallback(
    (sourcePaths: string[], targetPath: string, position: 'before' | 'after') => {
      ...
      const baseOrder =
        sortConfig.field === 'custom' && currentCustomOrder.length > 0
          ? currentCustomOrder
          : availablePaths;
      ...
      const targetIndex = withoutSources.indexOf(targetPath);
      let insertIndex = targetIndex === -1 ? withoutSources.length : targetIndex;
    ...
    <div className="bg-zinc-900 text-white h-screen flex flex-col font-sans">
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
        <Explorer
          ...
          onSelectSidebarFolder={handleSelectFolderFromSidebar}
          onSelectItem={handleItemClickWithScope}
          onSelectItemByIndex={handleSelectItemByIndex}
          onOpenLightbox={openLightboxAtFolderIndex}
          onSelectFavorite={handleSelectFavoriteFromSidebar}
      ...
      {selectedAoeItems.length >= 2 && (
        <div className="fixed bottom-6 right-6 z-30">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-fuchsia-500/40 bg-zinc-900/90 backdrop-blur">
      ...
      {contextMenu && (
        ...
        >
          <div
            className="absolute bg-neutral-800 border border-neutral-700 rounded-md py-1 min-w-[160px] text-sm"
