...
const NavItem: React.FC<{
    ...
    onMoveItem?: (sourcePath: string, destinationDir: string) => void;
    isDragActive: boolean;
}> = ({ icon, label, onClick, isSelected, path, onMoveItem, isDragActive }) => {
...
const FileTree: React.FC<FileTreeProps> = ({ rootPath, folderTree, selectedPath, onSelect, onSelectFavorite, onMoveItem, isDragActive }) => {
            ...
            <nav className="flex flex-col space-y-4 h-full">
                <div>
                    ...
                    <ul className="space-y-1">
                        <NavItem icon={<DesktopIcon />} label="Desktop" onClick={() => onSelectFavorite('desktop')} isSelected={false} isDragActive={false} />
                        <NavItem icon={<DocumentsIcon />} label="Documents" onClick={() => onSelectFavorite('documents')} isSelected={false} isDragActive={false} />
                        <NavItem icon={<PicturesIcon />} label="Pictures" onClick={() => onSelectFavorite('pictures')} isSelected={false} isDragActive={false} />
                    </ul>
                </div>
